...
class PyGameDisplay(busdisplay.BusDisplay):
    ...
    def _refresh_area(self, area) -> bool:
        ...
        def rgb_to_surface(buff, size):
            ...
            # arr = np.frombuffer(buff, dtype=np.uint16).newbyteorder('S')
            # arr = np.frombuffer(buff, dtype=np.uint16).view(np.uint16().newbyteorder('S'))
            dtype_swapped = np.dtype(np.uint16).newbyteorder("S")
            arr = np.frombuffer(buff, dtype=np.uint16).view(dtype_swapped)
